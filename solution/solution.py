...
class LLMJudge:
    ...
    def score_response(
        ...
        scores = {}
        try:
            # We look for a JSON-like substring in case there is surrounding text
            match = re.search(r"\{.*\}", raw_response, re.DOTALL)
            if match:
                data = json.loads(match.group(0))
                # For each criterion in the rubric, see if it is in the data
                for criterion in rubric.keys():
                    if criterion in data:
                        scores[criterion] = float(data[criterion])
                    else:
    ...
    def detect_bias(self, scores_batch: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        if len(scores_batch) <= 1:
            ...
            for item in scores_batch[1:]:
                ...
                if scores:
                    rest_avgs.append(sum(scores) / len(scores))
            rest_avg = sum(rest_avgs) / len(rest_avgs) if rest_avgs else 0.0
...
class BenchmarkRunner:
    ...
    def run_regression(self, new_results: list, baseline_results: list) -> dict:
        ...
        new_f = sum(r.faithfulness for r in new_results) / n if n > 0 else 0.0
        new_r = sum(r.relevance for r in new_results) / n if n > 0 else 0.0
        new_c = sum(r.completeness for r in new_results) / n if n > 0 else 0.0

        base_f = sum(r.faithfulness for r in baseline_results) / b if b > 0 else 0.0
        base_r = sum(r.relevance for r in baseline_results) / b if b > 0 else 0.0
        base_c = sum(r.completeness for r in baseline_results) / b if b > 0 else 0.0
...
class FailureAnalyzer:
    ...
    def generate_improvement_log(self, failures: list, suggestions: list[str]) -> str:
        ...
        for idx, f in enumerate(failures):
            ...
            suggest = suggestions[idx] if idx < len(suggestions) else "N/A"
            lines.append(f"| {fid} | {ftype} | {rcause} | {suggest} | Open |")
        return "\n".join(lines)
    ...
    def generate_improvement_suggestions(
        ...
        if categories.get("irrelevant", 0) > 0:
            suggestions.append("Improve prompt clarity and instruct the agent to stay focused on the user query")

        # Add default high-priority RAG recommendations to ensure at least 3 suggestions
        if len(suggestions) < 3:
            suggestions.append("Increase chunk size in RAG pipeline to reduce context fragmentation")
        if len(suggestions) < 3:
            suggestions.append("Integrate re-ranking (e.g. Cohere Rerank) to improve retrieval precision")
        if len(suggestions) < 3:
            suggestions.append("Refine system prompts with strict negative constraints to prevent out-of-scope answers")

        return suggestions[:max(3, len(failures))]
